...
public class ProducerManager {
    ...
    public synchronized void doChannelCloseEvent(String remoteAddr, Channel channel) {
        if (channel != null) {
            ...
            while(var3.hasNext()) {
                ...
                String group = (String)entry.getKey();
                ConcurrentHashMap<Channel, ClientChannelInfo> clientChannelInfoTable = (ConcurrentHashMap)entry.getValue();
    ...
    public synchronized void registerProducer(String group, ClientChannelInfo clientChannelInfo) {
        ...
        ConcurrentHashMap<Channel, ClientChannelInfo> channelTable = (ConcurrentHashMap)this.groupChannelTable.get(group);
    ...
    public Channel getAvaliableChannel(String groupId) {
        if (groupId == null) {
            ...
            ConcurrentHashMap<Channel, ClientChannelInfo> channelClientChannelInfoHashMap = (ConcurrentHashMap)this.groupChannelTable.get(groupId);
            if (channelClientChannelInfoHashMap == null) {
                ...
                if (0 == size) {
                    ...
                    for(boolean isOk = channel.isActive() && channel.isWritable(); count++ < 3; isOk = channel.isActive() && channel.isWritable()) {
                        if (isOk) {
                            return channel;
                        }
