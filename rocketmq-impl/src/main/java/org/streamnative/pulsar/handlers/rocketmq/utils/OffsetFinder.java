 ...
 */
@Slf4j
public class OffsetFinder implements FindEntryCallback {
    ...
    public void findMessages(final long timestamp, FindEntryCallback callback) {
        ...
        if (messageFindInProgressUpdater.compareAndSet(this, FALSE, TRUE)) {
            ...
            asyncFindNewestMatching(FindPositionConstraint.SearchAllAvailableEntries, entry -> {
                MessageImpl msg = null;
                try {
                    msg = MessageImpl.deserialize(entry.getDataBuffer());
                    return msg.getPublishTime() <= timestamp;
                } catch (Exception e) {
                    log.error("Error deserializing message for message position find, {}", e.getMessage());
                } finally {
                    entry.release();
                    if (msg != null) {
                        msg.recycle();
    ...
    public void findEntryComplete(Position position, Object ctx) {
        checkArgument(ctx instanceof FindEntryCallback);
        FindEntryCallback callback = (FindEntryCallback) ctx;
        if (position != null) {
